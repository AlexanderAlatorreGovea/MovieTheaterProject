...
class SearchField extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps() {
    this.props.searchData(
      `https://api.themoviedb.org/3/search/movie?api_key=${this.apiKey}&query=${this.props.match.params.id}`
    );
  }
      ...
      <div className="SearchField">
        <div className="MovieList" style={{ paddingLeft: "3rem" }}>
          <div className="searchField-title">
            <span>
              {this.props.searchDataResults.results.length} Results found
              For:&nbsp; {`${this.props.match.params.id}`}{" "}
            </span>
          </div>
          <div className="MovieList-Wrapper">
            {this.props.searchDataResults.results.length > 0 &&
              this.props.searchDataResults.results.map((item, i) => {
                return (
                  <div
                    className="MovieCard"
