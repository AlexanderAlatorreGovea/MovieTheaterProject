...
class RMDetails extends React.Component {
    ...
    getData = () => {
        ...
        axios.get(`https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${this.apiKey}&language=en-US&page=1`)
            .then(res => {
            this.setState({
    ...
    getPeopleData = () => {
        const { id } = this.props.location.state.movie;
        axios.get(`${baseUrl}${id}?api_key=${apiKey}&append_to_response=videos,details,images,movie_id`).then(res => {
            this.setState({
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.match.params.id !== this.props.match.params.id) {
            this.getNewData();
            this.getPeopleData();
            this.onLoad(prevProps.match.params.id, this.props.match.params.id)
        }
    }
