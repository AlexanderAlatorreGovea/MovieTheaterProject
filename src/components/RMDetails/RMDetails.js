...
class RMDetails extends React.Component {
    ...
    getData = async () => {
        ...
        await axios.get(`https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${this.apiKey}&language=en-US&page=1`)
            .then(res => {
                this.setState({
    ...
    getPeopleData = async () => {
        const { id } = this.props.location.state.movie;
        await axios.get(`${baseUrl}${id}?api_key=${apiKey}&append_to_response=videos,details,images,movie_id`)
            .then(res => {
                this.setState({
    ...
    async componentDidUpdate(prevProps) {
        if (prevProps.match.params.id !== this.props.match.params.id) {
            ...
            await this.getPeopleData();
            await this.onLoad(prevProps.match.params.id, this.props.match.params.id)
        }
    }
                    ...
                    <div className="movie-card" style={{ background: 'white' }}>
                        <div className="container">
                            <div className="container-wrapper">
                                <div className="secondary__image--video">
                                    <div className="secondary__image">
                                        <div className="secondary__image--wrapper">
                                            <img src={`${MovieDetailsConfig.imgURL}w780${secondaryImage}`} id="cover" alt="cover" className="cover"
                                ...
                                <div className="hero"
                                    style={{ background: `url( ${MovieDetailsConfig.imgURL}/w1280/${this.props.location.state.movie.poster_path})` }}>

                                    <div className="details">
                                        <div className="title__and--ratings">
                                            <div className="title1">{`${MovieDetailsConfig.movie.original_title}`}
