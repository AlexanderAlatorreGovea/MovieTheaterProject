...
class SearchBarModal extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=${this.apiKey}&query=${this.state.searchQuery}`
    )
      .then((data) => data.json())
      .then((data) => {
        this.setState({
      ...
      <div>
        <div className="SerachBar">
          <div className="search-item">
            <form
              className={`pa ${this.props.toggleSearchBar ? "show" : "hide"}`}
            ...
            >
              <div className="measure__">
                ...
                >
                  <button
                    type="submit"
                    className="main-nav-search-form__button"
                  >
                    <i className="white large search icon svg"></i>
                  </button>
                </Link>
              </div>
            </form>
