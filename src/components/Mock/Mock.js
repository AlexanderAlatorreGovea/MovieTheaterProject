    ...
    constructor(props) {
     ...
     allImgsLoop = () => {
         return this.state.allImgs.map((item, index) => {
             return(
                <div onClick={this.clickedThumb.bind(null, index)} key={index} className="image-1" style={{ backgroundImage: `url('${item}')`, height: '10vh', width: '150px' }}></div>
             )
         })
     }
        ...
        return (
            <div className='Mock' style={{height: '100vh', background: 'white'}}>
                <div className="slider">
                    <div style={{background: 'black', marginLeft: '8rem', height: '30px'}}  onClick={this.nextBtn}></div>
                    <div style={{background: 'red', marginLeft: '8rem', height: '30px'}} onClick={this.prevBtn} ></div>
                        <div className="image-1"
                        style={{ background: `url('${this.state.allImgs[this.state.currentIndex]}')`, height: '30vh', width: '400px'   }} >
                        </div>
                </div>
